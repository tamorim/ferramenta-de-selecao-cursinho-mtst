...
export default function App() {
  ...
  const [sortedFile, setSortedFile] = useState();
  ...
  const handleSearchRetrieve = (value) => {
  ...
  const handleUpload = async (event) => {
    const unparsedFile = await readUploadedFile(event.target.files[0]);
  ...
  const handleGetDirections = async () => {
    ...
    const retrievedPlaceIds = await Promise.all(
      uploadedFile.map(async (row, index) => {
        const [person, address] = row;
        if (!address) {
          return;
        }
    ...
    const retrievedDirections = await Promise.all(
      retrievedPlaceIds.map(async (placeId, index) => {
        if (!placeId) {
          return;
        }
